...
def display_line(window, string, location):
    ...
    # of where the string should displayed and it should be
...
def embed_password(password, size):
    ...
    # - size is the int number od characters in the padded
...
def display_password_list(window, location):
    # Display the embedded passwords, update the location for the next
    ...
    for password in password_list:
        ...
        embedding = embed_password(password, size)
...
def display_hint(window, password, guess, location):
    ...
    # coords of where the feedback should be displayed    
    ...
    for letter in guess:
        ...
        if (index < max) and (letter == password[index]):
            correct = correct + 1
        index = index + 1
    string_2 = str(correct) + '/' + str(max) + ' IN MATCHING POSITIONS'
    display_line(window, string_2, location)
